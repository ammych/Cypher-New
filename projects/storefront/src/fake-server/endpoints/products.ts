...
export function getStealDealsProducts(categorySlug: string, limit: number): Observable<Product[]> {
    limit = limit || 8;

    return delayResponse(of(clone(getProducts(12, categorySlug).slice(0, limit))), 1000);
}

export function getCatProducts(categorySlug: string, limit: number): Observable<Product[]> {
    limit = limit || 8;
